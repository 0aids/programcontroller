...
void TopLevelView::commitResponder(wl_listener *listener, void *data) {
  ...
  struct wlr_output *output = wlr_output_layout_get_center_output(
      self->m_parentServer->m_outputManager.m_outputLayout);
  if (!output) {
  ...
  wlr_output_layout_get_box(
      self->m_parentServer->m_outputManager.m_outputLayout, output,
      &output_box);
  ...
  wlr_scene_node_set_position(&self->m_sceneTree->node, output_box.x,
                              output_box.y);
  ...
  struct wlr_scene_node *node = wlr_scene_node_at(
      ...
      self->m_parentServer->m_inputManager.m_cursorManager.m_cursor->x,
      self->m_parentServer->m_inputManager.m_cursorManager.m_cursor->y, &sx,
      &sy);
  wlr_seat_pointer_notify_enter(self->m_parentServer->m_inputManager.m_seat,
                                self->m_topLevel->base->surface, sx, sy);

  auto *keyboard =
      wlr_seat_get_keyboard(self->m_parentServer->m_inputManager.m_seat);
  if (keyboard) {
    wlr_seat_keyboard_notify_enter(self->m_parentServer->m_inputManager.m_seat,
...
TopLevelView::TopLevelView(WaylandServer *parentServer,
                           wlr_xdg_toplevel *topLevel)
    : m_parentServer(parentServer), m_topLevel(topLevel) {
  ...
  m_sceneTree = wlr_scene_xdg_surface_create(
      &parentServer->m_sceneManager.m_scene->tree, topLevel->base);
...
void TopLevelsManager::newTopLevelPopupResponder(wl_listener *listener,
                                                 void *data) {
  // Do nothing
}
