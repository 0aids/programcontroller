...
BasicMemoryRegion::BasicMemoryRegion(const MemoryRegionProperties *properties)
...
e_ErrorTypes BasicMemoryRegion::snapshot(const pid_t &pid) {
  ...
  Log_var(Debug, name);
  Log_modevar(Debug, HEX_AND_SHOWBASE, startAddr);
  Log_modevar(Debug, HEX_AND_SHOWBASE, size);
  Log_var(Debug, "perms: " << maskToString(perms));
  ...
  if (nread < size) {
    if (nread == -1) {
      Log(Error, "Completely failed to read the region. Error is below.");
...
std::string RegionSnapshot::toStr() {
  ...
  Log_modevar(Debug, HEX_AND_SHOWBASE, this->size());
